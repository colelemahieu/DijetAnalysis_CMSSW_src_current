...
void forest2diJetSkim_pp5_new1(
		TString fname = "/eos/user/c/clemahie/HIForward/HiForward_Hiforest_PbPb_Run2018/220304_202625/0000/HiForestAOD_461.root",
		//TString fname = "/afs/cern.ch/work/a/abylinki/public/4_UPCTriggers_pp_reco_170427.root",
		//TString fname = "/afs/cern.ch/work/a/abylinki/public/Double_EG_test.root",
		TString outputFname = "upcDiJetSkimD1", 
		TString trig1 = "HLT_HIUPC_DoubleEG2_NotMBHF2AND_v1",  // "HLT_HIUPCL1SingleEG5NotHF2_v1",    // "HLT_HIUPCSingleEG5NotHF2Pixel_SingleTrack_v1" //  "HLT_HIUPCL1SingleEG5NotHF2_v1"
		TString trig = "HLT_HIUPC_SingleEG3_NotMBHF2AND_v1",  // "HLT_HIUPCL1SingleEG5NotHF2_v1",    // "HLT_HIUPCSingleEG5NotHF2Pixel_SingleTrack_v1" //  "HLT_HIUPCL1SingleEG5NotHF2_v1"
//		TString trig1 = "HLT_HIUPCL1DoubleMuOpenNotHF1_v1",  // "HLT_HIUPCL1SingleEG5NotHF2_v1",    // "HLT_HIUPCSingleEG5NotHF2Pixel_SingleTrack_v1" //  "HLT_HIUPCL1SingleEG5NotHF2_v1"
		//TString trig = "HLT_HIZeroBiasPixel_SingleTrack_v1",  // "HLT_HIUPCL1SingleEG5NotHF2_v1",    // "HLT_HIUPCSingleEG5NotHF2Pixel_SingleTrack_v1" //  "HLT_HIUPCL1SingleEG5NotHF2_v1"
		TString jetCollection = "akPu4PFJetAnalyzer", // "akPu5PFJetAnalyzer",
		float minjPt = 0,
		int nevt=-1
//	int nevt=100000
		) {  
...
//	TH1F *heff = (TH1F*)f2->Get("h4");
	...
	TTree *hbhe = (TTree*)f1->Get("rechitanalyzerpp/hbhe");
	TTree *hf = (TTree*)f1->Get("rechitanalyzerpp/hf");
	TTree *ee = (TTree*)f1->Get("rechitanalyzerpp/ee");
	TTree *eb = (TTree*)f1->Get("rechitanalyzerpp/eb");
	//  TTree *akpu5pf = (TTree*)f1->Get("t");
	//  TTree *pfTree  = (TTree*)f1->Get("pfcandAnalyzer/pfTree");
	TString dayTime = getDayAndTime();
	TFile* newfile = new TFile(Form("SkimmedForest/TrkCutsppreco_%s_trig%s_jetCollection%s_minJetPt%d_%s.root",outputFname.Data(), trig.Data(), jetCollection.Data(), (int)minjPt, dayTime.Data()  ),"recreate");
	...
	if (trig == "" ) {  cout << " No Trigger selection! " << endl ;}     
		...
		HltTree->SetBranchAddress(trig1.Data(), &trigBit1, &b_trigBit1);
	}
	...
	Float_t         hbee[10000];   
	Float_t         hbeet[10000];   
	Float_t         hbeeta[10000];   
	Float_t         hbephi[10000];   
	Float_t         hbeperp[10000];   
	Bool_t          hbeisjet[10000];   
	Int_t           hbedepth[10000];   
	TBranch        *b_hben;   
	TBranch        *b_hbee;   
	TBranch        *b_hbeet;   
	TBranch        *b_hbeeta;   
	...
	Float_t         hfeta[10000];
	Float_t         hfphi[10000];
	Float_t         hfperp[10000];
	Bool_t          hfisjet[10000];
	Int_t           hfdepth[10000];
	...
	Float_t         eeeta[10000];
	Float_t         eephi[10000];
	Float_t         eeperp[10000];
	Bool_t          eeisjet[10000];
	TBranch        *b_een;
	TBranch        *b_eee;
	TBranch        *b_eeet;
	TBranch        *b_eeeta;
	...
	Float_t         ebeta[10000];
	Float_t         ebphi[10000];
	Float_t         ebperp[10000];
	Bool_t          ebisjet[10000];
	TBranch        *b_ebn;
	TBranch        *b_ebe;
	TBranch        *b_ebet;
	TBranch        *b_ebeta;
	...
	eventTree->Branch("EG5",&EG5,"EG5/I");
	//   eventTree->Branch("
	...
	Float_t         HBHEperp[10000];   
	Bool_t          HBHEisjet[10000];   
	Int_t           HBHEdepth[10000];   
	Float_t         HBmax;
	Float_t         HEmax;
	Int_t           HFn;
	Float_t         HFe[10000];
...
//		cout << eg5<<endl;
		...
		if(HFplus_max >= 4.9 || HFminus_max >= 4.2)
		{
		//	continue;
		}
		//     if(HFplus_max >= 3.9)
		...
		for(Int_t p = 0; p != nVtx; p++)
		{
			...
			yVertex[p] = yVtx[p];
			zVertex[p] = zVtx[p];
			if (TMath::Abs(zVertex[p])>15) continue;
			if (TMath::Sqrt(xVertex[p]*xVertex[p]+yVertex[p]*yVertex[p])>.15) continue;
		}  
